export default defineEventHandler(async () => {
	...
	try {
		const response = await fetch(
			`https://graph.facebook.com/v18.0/${facebookPageId}/posts?fields=id,message,created_time,full_picture,permalink_url,from{name,picture}&limit=10&access_token=${config.facebookAccessToken}`,
			{
				headers: {
