...
import TableOfContents from "~/components/primitive/TableOfContents.vue";
...
const { data: post, pending } = await useFetch<BlogPostDetail>(
	`/api/blog/${slug}`,
);
...
<template>
	<NuxtLayout name="blog">
		<template #hero>
			<div
				v-if="post"
				class="h-screen flex items-center justify-center px-1.5rem"
			>
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-4rem max-w-7xl mx-auto items-center">
					...
					<div class="order-2 lg:order-2 text-left">
						...
						<p
							v-if="post.excerpt"
							class="text-1.25rem text-muted-foreground mb-1.5rem leading-relaxed"
						>
							{{ post.excerpt }}
						</p>
		...
		<div v-else class="text-center py-4rem text-muted-foreground">
			...
			<NuxtLink to="/blog" class="text-primary no-underline hover:underline"
