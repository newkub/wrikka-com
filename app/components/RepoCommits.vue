	...
	<div class="mt-0.75rem flex flex-col gap-0.5rem">
		<div class="flex items-center justify-between px-0 py-0.5rem">
			<div class="flex items-center gap-0.5rem">
				<Icon name="mdi:source-commit" class="w-1rem h-1rem text-muted-foreground" />
				<span class="text-0.875rem font-600 text-muted-foreground">Recent commits</span>
			</div>
			<span class="text-0.75rem text-muted-foreground">{{ formatTimeAgo(updatedAt) }}</span>
		</div>
		<a
		...
		>
			...
			<div class="flex-1 min-w-0">
				<p
					class="line-clamp-1 group-hover:text-primary transition-colors-0.2s m-0"
				>{{ commit.message }}</p>
				<span class="text-0.7rem text-muted-foreground">{{ formatTimeAgo(commit.date) }}</span>
			</div>
		</a>
	</div>
</template>
