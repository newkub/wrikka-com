...
onMounted(async () => {
	const highlighter = await createHighlighter({
		...
		langs: ['typescript', 'javascript', 'vue', 'bash', 'json', 'markdown', 'python', 'rust', 'go'],
	})
	...
	md.value = MarkdownIt({
		...
		typographer: true,
		highlight: (code, lang) => {
			if (!lang || !highlighter.getLoadedLanguages().includes(lang as BundledLanguage)) {
				return `<pre class="shiki"><code>${md.value!.utils.escapeHtml(code)}</code></pre>`
			}
