...
<template>
	<aside class="fixed top-0 right-0 w-[280px] h-screen overflow-y-auto border-l border-border bg-background p-6 hidden xl:block">
		<div class="text-sm font-semibold text-foreground mb-4 pb-2 border-b border-border">On this page</div>
		<nav>
			<ul v-if="tocItems.length > 0" class="list-none p-0 m-0">
				<li
					v-for="item in tocItems"
					:key="item.id"
					:class="['mb-1', item.level === 3 ? 'pl-4' : '', item.level === 4 ? 'pl-8' : '', { 'text-primary font-medium': activeId === item.id }]"
				>
					<NavLink
						:to="`#${item.id}`"
