...
const sidebarItems = computed<SidebarItem[]>(() => {
	...
	if (!set) {
		return docs.value
			...
			.filter(item => typeof item._path === 'string' && item._path.split('/').filter(Boolean).length === 3)
...
<template>
	<div class="relative">
		...
		<div v-if="isOpen" class="hidden fixed inset-0 z-[1000] bg-black/50 md:block" @click="closeSidebar"></div>

		<aside class="fixed top-[64px] left-0 w-[280px] h-[calc(100vh-64px)] overflow-y-auto border-r border-border bg-background p-6 transition-transform duration-300 md:translate-x-0" :class="{ 'translate-x-0': isOpen, '-translate-x-full': !isOpen }">
			<div class="flex items-center justify-between mb-8">
				<NuxtLink to="/docs" class="flex items-center gap-3 text-xl font-semibold text-foreground no-underline" @click="closeSidebar">
				...
				<div class="flex items-center gap-2">
					...
					<button @click="closeSidebar" class="hidden flex items-center justify-center w-9 h-9 border-none bg-transparent text-foreground rounded-lg cursor-pointer transition-all hover:bg-accent md:flex" aria-label="Close menu">
				...
				<ul class="list-none p-0 m-0">
					<li v-for="item in sidebarItems" :key="item.path">
						<NuxtLink
							...
							:class="isActive(item.path) ? 'bg-gradient-to-br from-primary/10 to-primary/5 text-primary font-600 border border-primary/20 hover:bg-gradient-to-br hover:from-primary/15 hover:to-primary/8' : ''"
							@click="handleNavClick(item.path)"
						>
							<div class="flex items-center justify-between gap-2">
								<span class="flex-1">{{ item.title }}</span>
								<Icon v-if="isActive(item.path)" name="mdi:check" size="16" class="flex-shrink-0 opacity-80" />
							</div>
						</NuxtLink>
					</li>
				</ul>
