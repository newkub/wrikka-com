...
const { data: activity, pending } = await useFetch<ActivityData>("/api/github/activity")
	...
	<div v-else-if="activity" class="flex flex-col gap-0.5rem">
		...
		<div class="overflow-x-auto">
			<div class="flex gap-0.25rem">
				<div v-for="(week, weekIndex) in activity.weeks" :key="weekIndex" class="flex flex-col gap-0.25rem">
					<div
		...
		<div class="flex items-center gap-0.5rem mt-0.5rem text-0.75rem text-gray-600 dark:text-gray-400">
			...
			<div class="flex gap-0.125rem">
				<div class="w-0.75rem h-0.75rem rounded-sm bg-gray-100 dark:bg-gray-800"></div>
				<div class="w-0.75rem h-0.75rem rounded-sm bg-green-200 dark:bg-green-900"></div>
				<div class="w-0.75rem h-0.75rem rounded-sm bg-green-300 dark:bg-green-800"></div>
				<div class="w-0.75rem h-0.75rem rounded-sm bg-green-400 dark:bg-green-700"></div>
				<div class="w-0.75rem h-0.75rem rounded-sm bg-green-500 dark:bg-green-600"></div>
			</div>
			<span>More</span>
		</div>
	</div>
