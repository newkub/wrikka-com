...
const { data: doc, error } = await useAsyncData(`doc:${path.value}`, async () => {
...
const readingTimeContent = computed(() => {
	...
	const content = typeof doc.value.body === 'string' ? doc.value.body : JSON.stringify(doc.value.body)
	return content
})

const { readingTime, formatReadingTime } = useReadingTime(readingTimeContent.value)
	...
	<NuxtLayout name="docs">
		<div class="mb-6 pb-4 border-b border-border">
			<div class="flex items-center gap-6 flex-wrap">
				<span v-if="doc?.updatedAt" class="flex items-center gap-2 text-sm text-muted-foreground">
